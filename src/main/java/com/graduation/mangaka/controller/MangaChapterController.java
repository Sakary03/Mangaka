...
public class MangaChapterController {
    ...
    @GetMapping("/get-all")
    public ResponseEntity<?> getAllChapters(@RequestParam(value= "offset", defaultValue = "0") int offset,
                                            @RequestParam(value= "limit", defaultValue = "10") int limit,
                                            @RequestParam(value = "sortField", defaultValue = "createdAt") String sortField) {
        return ResponseEntity.ok().body(mangaChapterService.getChapterByBatch(offset, limit, sortField));
    }
}
